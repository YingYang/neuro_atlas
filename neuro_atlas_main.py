...
class Analyses:
    ...
    cv_lda = "cv_lda"
    individual_lda = "individual_lda"
    cv_gnb = "cv_gnb"
    individual_gnb = "individual_gnb"
...
if __name__ == '__main__':
    ...
    if parsed_arguments.analysis in [Analyses.cv_lda, Analyses.cv_gnb,
        ...
        Analyses.individual_lda, Analyses.individual_gnb,
        Analyses.individual_logistic_regression, Analyses.individual_knn,
        Analyses.individual_svm]:
         
        # ====determine whether to load individual data or not ===
        flag_individual_test = False
        if parsed_arguments.analysis in [Analyses.individual_lda,
            Analyses.individual_gnb, Analyses.individual_logistic_regression, 
            Analyses.individual_knn, Analyses.individual_svm]:
            flag_individual_test = True
        
        # compute features and labels from the avrg data
        ...
        # =====if test on  an individual subject, extract features and labels
        # from the individual =============
        if flag_individual_test:
        ...
        if parsed_arguments.analysis in [Analyses.cv_lda, Analyses.individual_lda]:
            estimators = [LDA(solver = "lsqr", shrinkage = None)]
            estimator_names = ["LDA"]
        elif parsed_arguments.analysis in [Analyses.cv_gnb, Analyses.individual_gnb]:
            estimators = [GaussianNB()]
            estimator_names = ["GaussianNaiveBayes"]
        elif parsed_arguments.analysis in [Analyses.cv_logistic_regression,
                                           Analyses.individual_logistic_regression]:
            estimators = [LogisticRegression(multi_class="ovr", penalty='l2',
                                solver='sag', max_iter=1000, n_jobs=-1)]
            estimator_names = ["LogisticRegressionL2"]
        elif parsed_arguments.analysis in [Analyses.cv_knn, Analyses.individual_knn]:
            estimators = [neighbors.KNeighborsClassifier(n_neighbors=1)]
            estimator_names = ["1NN"]
        elif parsed_arguments.analysis == Analyses.cv_svm:
            estimators = [SVC(kernel="linear"),
                        SVC(kernel = "rbf", gamma=0.0000082)]
            estimator_names = ["SVM_Linear","SVM_RBF"]
        
        for estimator_id in range(len(estimators)):
            t0 = time.time()
            estimator = estimators[estimator_id]    
                
            if not flag_individual_test: 
                # cross validation         
                ...
                print(estimator_names[estimator_id])        
